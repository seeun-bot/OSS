...
$\text{Git Stash}$는 작업 중이던 변경사항을 **잠시 보관**해 두고 작업 디렉토리를 깨끗하게 비우는 기능입니다. 갑자기 다른 브랜치로 이동해야 할 때 매우 유용합니다.

*   **기본 명령:** `git stash`
*   **목록 확인:** `git stash list`
*   **역할:** 작업 디렉토리($\text{Working Directory}$)와 스테이징 영역($\text{Staging Area}$)을 일시적으로 비워줍니다.

### 📌 적용 및 제거 옵션

| 명령어 | 기능 | 설명 |
| :--- | :--- | :--- |
| `git stash apply` | 적용 ($\text{Apply}$) | $\text{Stash}$ 내용을 불러오지만, $\text{stash}$ 목록에서는 **삭제하지 않습니다**. |
| `git stash pop` | 적용 후 제거 ($\text{Pop}$) | $\text{Stash}$ 내용을 적용하고, $\text{stash}$ 목록에서 해당 항목을 **제거합니다**. |

### 💡 고급 옵션

*   `--index`: 스테이징 영역에 있던 내용도 함께 임시 저장합니다.
*   `-u` (또는 `--include-untracked`): $\text{Untracked}$ 파일($\text{Git}$이 추적하지 않던 파일)까지도 함께 임시 저장합니다.

---

## 🌳 8-1 다양한 브랜치 병합 ($\text{Merge}$)

$\text{Git}$ 병합의 기본인 **$\text{fast-forward merge}$**와 **$\text{3-way merge}$**의 차이점을 다루는 개념입니다.

### 🔄 병합의 두 가지 유형

| 유형 | 조건 | 결과 | 특징 |
| :--- | :--- | :--- | :--- |
| **$\text{Fast-forward Merge}$** | 브랜치 이력이 일렬 구조일 때 (합칠 브랜치가 현재 브랜치의 후손일 때) | 새로운 $\text{merge commit}$ 없이 **브랜치 포인터만** 이동 | 기본적으로 수행되는 가장 빠른 병합 방식 |
| **$\text{3-way Merge}$** | 두 브랜치가 서로 다른 조상 ($\text{commit}$)을 가지고 있을 때 | 새로운 $\text{merge commit}$ 생성 후 내용 통합 | 두 이력을 합치고 병합 기록을 남김 |

---

## 🛠️ 8-2 [실습] 다양한 브랜치 병합

개념 학습을 기반으로 $\text{fast-forward}$ 여부를 실제 실습을 통해 확인합니다.

*   **실습 내용:** 여러 브랜치에서 다른 내용을 $\text{Commit}$하고 병합을 실행하여 $\text{Git}$ 로그를 확인.
*   **핵심 옵션:** `git merge --no-ff`
    *   **기능:** $\text{fast-forward}$가 가능한 상황에서도 강제로 **새로운 $\text{merge commit}$**을 생성하여 병합 기록을 명확히 남깁니다.

---

## 💥 8-3 병합 충돌과 해결 ($\text{Conflict Resolution}$)

두 브랜치에서 **같은 파일의 같은 부분**을 서로 다르게 수정했을 때 발생하는 상황입니다.

*   **충돌 표시:** $\text{Git}$이 충돌 부분을 파일 내에 다음과 같은 표시자로 알려줍니다.
    *   `<<<<<<< (현재 브랜치 내용)`
    *   `======= (구분선)`
    *   `>>>>>>> (병합할 브랜치 내용)`
*   **해결 방법:**
    1.  개발자가 파일을 열어 원하는 최종 내용을 직접 선택하고 수정합니다.
    2.  수정된 파일을 $\text{Staging Area}$에 추가 (`git add`).
    3.  $\text{Commit}$을 통해 병합을 **마무리** (`git commit`).

---

## ⚙️ 9-1 브랜치 리베이스 ($\text{Rebase}$)

브랜치의 기반($\text{base}$)을 다른 커밋 위로 **다시 이동**시키는 기능입니다. 이력을 깔끔하게 정리하는 데 사용됩니다.

*   **차이점 ($\text{vs Merge}$):** $\text{merge commit}$을 만들지 않고 커밋 이력을 **일렬($\text{linear}$)**로 정리합니다.
*   **사용 시 주의 사항:**
    *   $\text{Rebase}$는 기존 커밋 기록을 **변경**합니다.
    *   이미 팀원들과 **공유된 브랜치**를 $\text{rebase}$하면 이력 불일치로 인해 큰 문제가 발생할 수 있으므로 **개인 작업 브랜치**에만 사용하는 것이 안전합니다.

---

## ✏️ 커밋 이력 수정

### 1. 최신 커밋 수정 ($\text{amend}$)

*   **메시지 수정:** `git commit --amend -m "새로운 메시지"`
*   **파일 추가:** 파일을 수정한 후 $\text{add}$하고 `git commit --amend`를 실행하면 최신 커밋에 해당 변경 내용을 포함시킬 수 있습니다.

### 2. 여러 커밋 수정 ($\text{rebase -i}$)

*   **명령어:** `git rebase -i HEAD~N` (최신 $\text{N}$개의 커밋을 대상으로)
*   **활용:** $\text{Interactive Rebase}$를 통해 `pick`, `reword` (메시지 수정), `squash` (커밋 합치기) 등의 명령어를 사용하여 **여러 커밋**의 이력을 재작성 및 정리할 수 있습니다.

---

## 🖥️ 비주얼 스튜디오 코드 ($\text{VS Code}$)에서 Git 활용

$\text{VS Code}$는 내장된 기능을 통해 $\text{Git}$을 **GUI**로 쉽게 다룰 수 있게 지원합니다.

*   **기본 기능:**
    *   **저장소 초기화:** 폴더를 연 후 저장소 초기화 ($\text{Initialize Repository}$).
    *   **변경 사항 확인:** 변경 사항($\text{Changes}$)과 스테이지($\text{Staged Changes}$)를 소스 제어 뷰에서 시각적으로 확인 및 관리.
    *   **파일 비교 ($\text{diff}$):** 이전 버전 또는 $\text{Staged}$된 내용과 현재 내용을 매우 편리하게 비교.
*   **확장:** **$\text{Git Graph}$**와 같은 확장을 설치하여 $\text{Git}$ 히스토리를 시각적으로 확인할 수 있습니다.

---

## 🔙 버전 되돌리기 (reset) – 개념 정리

커밋 포인터를 과거로 이동시켜 버전을 되돌리는 기능입니다.

| 옵션 | $\text{HEAD}$ (커밋) | $\text{Staging Area}$ (Index) | $\text{Working Directory}$ | 특징 |
| :--- | :--- | :--- | :--- | :--- |
| **`--soft`** | 이동 | 유지 | 유지 | 커밋만 되돌리고 변경 내용은 유지 |
| **`--mixed` (기본값)** | 이동 | 초기화 | 유지 | 스테이지된 내용까지 초기화 |
| **`--hard`** | 이동 | 초기화 | 초기화 | **모든 변경 삭제** (되돌릴 수 없음) |

### 🛠️ [실습] 버전 되돌리기 ($\text{reset}$)

*   **목표:** 각 $\text{reset}$ 옵션이 $\text{Working Directory}$, $\text{Staging Area}$, $\text{Commit Log}$에 어떤 변화를 주는지 $\text{git log}$와 $\text{git status}$를 함께 보면서 상태 변화를 확인합니다.

---

## ↩️ 커밋 취소 ($\text{revert}$)

이미 배포되거나 공유된 브랜치에서 커밋을 취소할 때 사용하는 **안전한** 방법입니다.

*   **개념:** 기존 기록을 **삭제하지 않고**, 취소하려는 커밋의 작업과 **반대되는 작업**을 수행하는 **새로운 커밋**을 생성합니다.
*   **특징:**
    *   협업 환경에서 가장 안전한 롤백($\text{Rollback}$) 방식입니다.
    *   기존 커밋 이력을 **보존**하므로 히스토리가 투명하게 유지됩니다.
    *   여러 개의 커밋도 순차적으로 $\text{revert}$하여 취소할 수 있습니다.
